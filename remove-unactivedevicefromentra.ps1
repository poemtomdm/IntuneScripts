...
$userMembershipUri = "$url$posturl"
try {
    $isMember = $true
    $checkMembership = Invoke-MgGraphRequest -Uri $userMembershipUri -Method GET -ErrorAction SilentlyContinue

    # If the user is not a member, $checkMembership would be null or empty
    if (-not $checkMembership) {
        $isMember = $false
    }
}
catch {
    $isMember = $false  # Assume user is not a member if there's an error
}

# If user is not a member, proceed with adding them to the group
if (-not $isMember) {
    try {
        ...
        Write-Host "Error adding user $userid to group:"
    }
}

else {
    Write-Host "User $userid is already a member of the group."
}
        ...
        $groupname=$groupmemberof.displayname
            if ($groupname -like "*ORG0000-SG-MAC-USR-KerberosExtension*" -and $groupname -notlike "*$userdomain*") {
                ...
                try {
                    ...
                    Write-Host "Error removing user $userid from group $groupname"
                }
            }
    }
